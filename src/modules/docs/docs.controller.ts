import { Controller, Get, Put, Post, Delete, Param, Body, UnauthorizedException,
  ParseUUIDPipe,
} from '@nestjs/common';
...
export class DocsController {
  ...
  getVersions(@CurrentUser() user: User, @Param('repoId', ParseUUIDPipe) repoId: string) {
    return this.service.getVersions(user.id, repoId);
  }

  @Get(':repoId/docs')
  getDoc(@CurrentUser() user: User, @Param('repoId', ParseUUIDPipe) repoId: string) {
    return this.service.getLatestDoc(user.id, repoId);
  }

  @Get(':repoId/docs/status')
  getStatus(@CurrentUser() user: User, @Param('repoId', ParseUUIDPipe) repoId: string) {
  ...
  generate(@CurrentUser() user: User, @Param('repoId', ParseUUIDPipe) repoId: string) {
  ...
  refresh(@CurrentUser() user: User, @Param('repoId', ParseUUIDPipe) repoId: string) {
    if (!user || !user.id) throw new UnauthorizedException('กรุณาเข้าสู่ระบบก่อน');
  ...
  autoUpdate(@CurrentUser() user: User, @Param('repoId', ParseUUIDPipe) repoId: string) {
    if (!user || !user.id) throw new UnauthorizedException('กรุณาเข้าสู่ระบบก่อน');
    return this.service.autoUpdate(user.id, repoId);
  }

  @Delete(':repoId/docs')
  deleteDoc(@CurrentUser() user: User, @Param('repoId', ParseUUIDPipe) repoId: string) {
